			...
			<div class="col-md-6" style="margin:0px auto;float:none;" ng-app='myApp' ng-controller='myCtrl'>
				...
				<div class="panel">
					<div class="panel-body">
						...
						<input type="text" class="form-control" placeholder="state name" ng-model="stateName">
						<label>Region </label>
						<select name="" id="" ng-model="region.region" class="form-control">
							<option ng-repeat="x in regions" value="{{x.region_id}}">{{x.region_name}}</option>
						</select>
						<br>
						<br>
						<label>State Description</label>
						<textarea class="form-control" placeholder="state description" rows="4" ng-model="stateDescription"></textarea>

						<div>
							<img ng-src="{{image.image}}" alt="" style="max-height:200px;">
						</div>
						...
						<button type="button" class="btn btn-primary btn-sm" ng-click="submit($event)"> Edit</button>
					</div>
				</div>
