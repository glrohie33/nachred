...
class Home extends CI_Controller
{
	...
	public function dam($slug = null)
	{
		...
		if ($state->num_rows() > 0) {
			...
			$this->db->select('dam_id,dam_name,dam_description,views, dam.update_at,dam.created_at,dam.feature_img,dam.images,dam.location,dam.ownership,dam.hydrology,dam.physio,region.region_name,state.state_name');
			$this->db->from('dam');
			...
			$view = $dams->views + 1;
			$id = $dams->dam_id;
			$this->db->where('dam_id', $id);
			$this->db->update('dam', array('views' => $view));
			$this->load->view('home/index', $page_data);
		}
	}
}
